...
void setpixel(uint8_t *buff, uint8_t x, uint8_t y, uint8_t color) {
	if (x > 128 || y > 64) {
		printf("Error: Drawing operation exceeds bounds"); 
		return; 
	} 
	
	buff[x + 128*(y/8)] |= 1<<(7-(y%8)); 
}
...
void drawstring(uint8_t *buff, uint8_t x, uint8_t line, uint8_t *c) {
	...
	while(*c) {
		drawchar(buff[x+(line*128)], x, line, *c); 
		*c++;  
		x = x+5; 
...
}//function to clear a rectangle from the screen
...
void drawrect(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color) {
	uint8_t dimensionx = w - x; 
	uint8_t dimensiony = h - y;
	int i,j;
	for (i = x; i <= w; i++) {
		setpixel(buff, i, y, 0x00);
		setpixel(buff, i, h, 0x00); 
	}
	for (j = y; j <= h; j++) {
		setpixel(buff, x, j, 0x00); 
		setpixel(buff, w, j, 0x00); 
	}
}

//helper function to help draw the circle
void drawall(uint8_t *buff,uint8_t xc, uint8_t yc, uint8_t x, uint8_t y, uint8_t color) {
	setpixel(buff, xc+x, yc+y, color); 
	setpixel(buff, xc-x, yc+y, color); 
	setpixel(buff, xc+x, yc-y, color); 
	setpixel(buff, xc-x, yc-y, color); 
	setpixel(buff, xc-y, yc-x, color); 
	setpixel(buff, xc+y, yc+x, color); 
	setpixel(buff, xc-y, yc+x, color); 
	setpixel(buff, xc+y, yc-x, color); 
}
...
void drawhorzline(uint8_t *buff, uint8_t start, uint8_t end, uint8_t line, uint8_t color) {
	for (int i = start; i <= end; i++) {
		setpixel(buff, i, line, color); 
	}
}
...
//helper function used in clearing the circle
void clearwall(uint8_t *buff,uint8_t xc, uint8_t yc, uint8_t x, uint8_t y, uint8_t color) {
	clearpixel(buff, xc+x, yc+y);
	clearpixel(buff, xc-x, yc+y);
	clearpixel(buff, xc+x, yc-y);
	clearpixel(buff, xc-x, yc-y);
	clearpixel(buff, xc-y, yc-x);
	clearpixel(buff, xc+y, yc+x);
	clearpixel(buff, xc-y, yc+x);
	clearpixel(buff, xc+y, yc-x);
}

//function to clear circle
void clearcircle(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t r,uint8_t color) {
		
		for (int y = -r; y <= r; y++) {
			for (int x = -r; x <= r; x++) {
				if (x*x + y*y <= r) {
					clearpixel(buff, x0 + x, y0 + y);
				}
			}
		}
...
void initializeADC() {
	ADMUX = (1<<REFS0);
	ADMUX &= ~(1<<REFS1);
	//set the ADC with prescaler of 128 and enable the ADC
	ADCSRA = (1<<ADEN)|(1<<ADPS2)|(1<<ADPS1)|(1<<ADPS0); 
	ADCSRB &= ~((1<<ADTS0)| (1<<ADTS1) | (1<<ADTS2)); //set the ADC to free-running mode

}

//function reaad the x coordinate
int readxcoordinate() {
	//set X- and X+ to digital mode
	DDRC |= (1<<PORTC0) | (1<<PORTC2);
	//set Y-and Y+ to input mode
	DDRC &= ~((1<<PORTC1) | (1<<PORTC3)); 
	//set Y- and Y+ to read ADC input mode
	PORTC &= ~((1<<PORTC3) | (1<<PORTC1));

	PORTC |= (1<<PORTC0); 	//set Y+ to high 
	PORTC &= ~(1<<PORTC2);  //set Y- to low
	
	//set ADMUX register to read from ADC3
	ADMUX &= 0xF0; 
	ADMUX |= (1<<MUX1) | (1<<MUX0); 
...
int readycoordinate() {
	...
	DDRC &= 0xF0; //reset the data direction register
	DDRC |= (1<<PORTC3)| (1<<PORTC1); //set Y- adn Y+ to output
	PORTC &= ~(1<<PORTC0); //Y- high
	PORTC |= (1<<PORTC3); //Y+ low
	PORTC &= ~(1<<PORTC1);  //X- low
